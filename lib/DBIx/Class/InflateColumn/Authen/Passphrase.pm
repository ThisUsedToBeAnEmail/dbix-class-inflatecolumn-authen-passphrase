...
sub register_column {
    ...
    my $class = 'Authen::Passphrase';
    if ($info->{passphrase_class}) {
         $class = sprintf '%s::%s', $class, $info->{passphrase_class};
    }
    load_class($class);
    ...
    $self->inflate_column(
        $column => {
            inflate => sub { $class->${\"from_${encoding}"}(shift, $_[0]->result_source->column_info($column)) },
